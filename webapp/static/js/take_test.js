...
submitButton.addEventListener("click", async (event) => {
    ...
    // Append audio files for each question
    questions.forEach((question, index) => {
        if (audioResponses.has(index)) {
            formData.append(`audio-${index}`, audioResponses.get(index), `question-${index}.webm`);
        }
    });

    // Optionally, add text data or metadata if needed
